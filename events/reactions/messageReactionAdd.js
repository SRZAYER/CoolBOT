module.exports = (client, messageReaction, user) => {
  ...
  if(["hipo", "trou"].includes(emoji) && message.channel.id === channel.id) {
   switch (emoji) {
    case "hipo":
      member.roles.add(hipoRole);
      message.channel.send(`Le rôle ${hipoRole.name} a été ajouté avec succès !`);
      break;
     case "trou":
      member.roles.add(trouRole);
      message.channel.send(`Le rôle ${trouRole.name} a été ajouté avec succès !`);
      break;
